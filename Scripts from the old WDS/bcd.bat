...
for /f "tokens=3" %%A in ('%BCDEDIT% /store %BCDSTORE% /create /d "Windows PE" /application osloader') do set guid=%%A

%BCDEDIT% /store %BCDSTORE% /set %guid% device partition=e:
%BCDEDIT% /store %BCDSTORE% /set %guid% osdevice partition=e:
%BCDEDIT% /store %BCDSTORE% /set %guid% path \Windows\system32\winload.exe
%BCDEDIT% /store %BCDSTORE% /set %guid% systemroot \Windows
